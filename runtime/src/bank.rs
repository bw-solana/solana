        ...
        HashMap::from_iter(
            self.rent_collection_partitions()
                ...
                .map(|(pubkey, account, slot)| {
                    let account_hash = AccountsDb::hash_account(slot, &account, &pubkey, IncludeSlotInHash::IncludeSlot);
                    (pubkey, account_hash)
                }),
        )
    }
    ...
    fn collect_rent_from_accounts(
        ...
        for (pubkey, account, loaded_slot) in accounts.iter_mut() {
                ...
                || !Self::skip_rewrite(rent_collected_info.rent_amount, account)
            {
                ...
                // feature is enabled.
                let hash = AccountsDb::hash_account(*loaded_slot, account, pubkey, IncludeSlotInHash::IncludeSlot);
