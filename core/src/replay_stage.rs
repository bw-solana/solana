...
use {
    dead_slots::{DeadSlotContext, DeadSlotDuplicateContext, DeadSlotNotifications},
    update_parent::ChildBankReplayStart,
};
        ...
        *replay_highest_frozen.highest_frozen_slot.lock().unwrap() = highest_frozen_slot;
        ...
        let run_replay = move || {
            ...
            loop {
                ...
                Self::handle_update_parent_interrupts(
                    &my_pubkey,
                    &blockstore,
                    &bank_forks,
                    &mut progress,
    ...
    fn generate_new_bank_forks(
        ...
        for (parent_slot, children) in next_slots {
            ...
            for child_slot in children {
                ...
                let replay_offset = match Self::child_bank_replay_start(
                    blockstore,
                    parent_bank,
                    parent_slot,
                    child_slot,
...
pub(crate) mod tests {
    ...
    fn test_update_parent_restart() {
        ...
        let (replay_vote_sender, replay_vote_receiver) = unbounded();
        let mut async_verification_freelist = Vec::new();
        ReplayStage::handle_update_parent_interrupts(
            &Pubkey::new_unique(),
            &blockstore,
            &bank_forks,
            &mut progress,
    ...
    fn test_soft_dead_restarts() {
        ...
        let (replay_vote_sender, _replay_vote_receiver) = unbounded();
        let mut async_verification_freelist = Vec::new();
        ReplayStage::process_soft_dead_slots(
            &Pubkey::new_unique(),
            &blockstore,
            &bank_forks,
            &None,
