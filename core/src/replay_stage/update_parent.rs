...
use {
    super::{
        ...
        dead_slots::{DeadSlotLogLevel, DeadSlotNotifications},
    },
    crate::{
    ...
    solana_ledger::{
        ...
        blockstore_processor::{AsyncVerificationProgress, BlockstoreProcessorError},
    },
...
impl ReplayStage {
    ...
    fn update_parent_replay_offset(slot: Slot, slot_meta: &SlotMeta, root: Slot) -> Option<u64> {
        ...
        (root <= parent_slot && parent_slot < slot)
            .then_some(u64::from(slot_meta.replay_fec_set_index))
    }
    ...
    pub(super) fn should_mark_soft_dead(
    ...
    fn try_restart_slot_from_update_parent(
        ...
        let Some(replay_fec_set_index) =
            Self::update_parent_replay_offset(slot, &slot_meta, bank_forks.read().unwrap().root())
        else {
            return false;
        };
        ...
        if bank
            .as_ref()
            .is_some_and(|bank| Self::leader_is_me(bank.leader_id(), my_pubkey))
        {
            return false;
        }
        ...
        info!(
            "{my_pubkey}: restarting slot {slot} from replay_fec_set_index {replay_fec_set_index} \
             via {source} (was at {current_num_shreds} shreds)",
        );
    ...
    pub(super) fn process_soft_dead_slots(
        ...
        for slot in soft_dead_slots {
            ...
            let replay_offset = Self::update_parent_replay_offset(
                slot,
                &slot_meta,
            ...
            if replay_offset.is_some() {
                Self::try_restart_slot_from_update_parent(
                    my_pubkey,
                    blockstore.as_ref(),
                    bank_forks,
                    progress,
            ...
            if slot_meta.has_update_parent() || slot_meta.is_full() {
                let (err, log_level) = if slot_meta.has_update_parent() {
                    (
                        "error: replay failed before UpdateParent, and the UpdateParent metadata \
                         is invalid"
                            .to_string(),
    ...
    pub(super) fn handle_abandoned_bank(
        ...
        let update_parent_ready = !process_active_banks_context.blockstore.is_dead(bank_slot)
            && process_active_banks_context
                ...
                .is_some_and(|slot_meta| {
                    Self::update_parent_replay_offset(
                        bank_slot,
                        &slot_meta,
                        bank_forks.read().unwrap().root(),
                    )
        ...
        Self::send_invalid_bank(bank, &process_active_banks_context.replay_vote_sender);
